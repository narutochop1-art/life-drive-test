...
function setCharacter(prefix){
 ...
 // 关键修复：之前使用 new Image()+loading='lazy'，但图片当时还没有插入 DOM。
 // lazy 图片在尚未插入文档、无法与视口建立交集时可能不会发起请求，导致一直停留在占位图。
 // 现在直接把 img 插入当前已显示的结果卡，并使用 eager（默认）加载。
 art.innerHTML='';
 ...
 art.appendChild(img);
 img.src=c.assetPath+(c.assetPath.includes('?')?'&':'?')+'v=10';
}
function result(){const c=comboData(t);$('comboIcon').textContent=c?.[1]||'✨';$('combo').textContent=c?.[0]||'复合驱动力者';$('tagline').textContent=c?.[2]||'';$('freeCore').textContent=c?.[3]||'';$('freeTension').textContent=c?.[4]||'';$('shareLine').textContent=c?.[8]||'';$('dims').innerHTML=t.map((k,n)=>`<div class="dim"><div class="dh"><b>${n+1}. ${D[k].icon} ${D[k].name}</b><span>${s[k]}</span></div><div class="track"><i style="width:${Math.min(100,s[k])}%"></i></div><small>${D[k].keywords}</small></div>`).join('');$('lockTitle').textContent='还有一部分结果没有告诉你';$('status').textContent='';$('invite').textContent='邀请朋友，解锁完整的我';$('copy').textContent='复制邀请链接';$('copy').style.display='block';show('result');setCharacter('');startUnlockPolling()}
...
function share(){const u=location.origin+location.pathname+'?invite='+encodeURIComponent(sid)+'&source=friend';ev('share_click',{method:navigator.share?'native_or_copy':'copy',combo:keyOf(t)});const text=`我测出来是「${combo(t)}」——${comboData(t)?.[8]||''}`;if(navigator.share){navigator.share({title:'你真正追求的是什么？',text,url:u}).then(()=>{$('status').textContent='邀请已发出，等朋友完成测试即可解锁。'}).catch(()=>{});return}navigator.clipboard?.writeText(u).then(()=>{$('status').textContent='邀请链接已复制，朋友完成测试后这里会自动解锁。'}).catch(()=>{$('status').textContent=u})}
function full(){const c=comboData(t);$('fullIcon').textContent=c?.[1]||'✨';$('fullcombo').textContent=c?.[0]||'复合驱动力者';$('fullTagline').textContent=c?.[2]||'';show('full');setCharacter('full');let html=`<article class="fullsec highlight"><small>你的核心组合</small><h2>${c?.[3]||''}</h2><p>${c?.[4]||''}</p></article>`;t.forEach((k,n)=>{html+=`<article class="fullsec"><small>TOP ${n+1} · ${D[k].label}</small><h2>${D[k].icon} ${D[k].name}</h2><p>${D[k].full}</p><h3>你为什么会被它驱动</h3><p>${D[k].core}</p><h3>你最容易掉进的坑</h3><p>${D[k].risk}</p>`});html+=`<article class="fullsec"><h2>你的核心取舍</h2><p>${c?.[4]||'在不同需求发生冲突时，你会倾向于保护排名更高的驱动力。'}</p><h2>别人最容易误解你的地方</h2><p>${c?.[5]||''}</p><h2>你容易被什么样的人吸引</h2><p>${c?.[6]||''}</p><h2>你的潜在盲区</h2><p>${c?.[7]||''}</p></article><article class="fullsec"><h2>最后一句话</h2><p class="bigquote">“${c?.[8]||''}”</p></article><article class="fullsec"><small>说明</small><p>这是一项探索性自测，用来帮助你观察自己在不同人生选择中的偏好，不是经过临床验证的心理诊断或人格测量工具。</p></article>`;$('fullbody').innerHTML=html;ev('full_view',{combo:keyOf(t)})}
$('start').onclick=()=>{i=0;a=[];ev('test_start');show('quiz');render()};$('quit').onclick=()=>show('home');$('invite').onclick=share;$('copy').onclick=share;$('restart').onclick=()=>show('home');$('again').onclick=()=>show('home');$('shareResult').onclick=share;
